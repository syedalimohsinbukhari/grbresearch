...
from .grb_constants import model_n_pars, MODEL_PARAMETERS
...
sbpl_params = ["amp_sbpl", "e_piv_sbpl", "index1_sbpl", "e_break_sbpl", "delta_sbpl", "index2_sbpl"]
...
cwd_ = os.getcwd()
dir_ = ["Ep0_Model_Data", "Ep1_Model_Data", "Ep2_Model_Data", "Ep3_Model_Data", "Ep4_Model_Data", "Ep5_Model_Data"]
...
def save_model(values: NDArray, model_name: str, ret: bool = False) -> Optional[pd.DataFrame]:
    df_ = pd.DataFrame(values).T
    df_.columns = MODEL_PARAMETERS[model_name]
    ...
    if not ret:
        df_clean.to_csv(path_or_buf=f"./{dir_[0]}/{model_name}.csv", index=False, float_format="%.17g")
        return None
    else:
        return df_clean
...
for fit_ in files_:
    ...
    with fits.open(f"{cwd_}/{dir_[0]}/{fit_}") as f:
        ...
        v = get_value(fit_file=f, n_parameters=model_n_pars[model_], full_cov=f[2].data["COVARMAT"][0])
        if model_ == "pl_bb":
