...
#         self._by_name: Dict[str, GRBModel] = {m.name: m for m in self._models}
#
#     # ------------------
#     # Core access
#     # ------------------
...
#
#     def get_parameter(self, par_name, mode="both", exclude_ex=False):
#         """
#         mode: 'both', 'value', 'error'
#         """
#         pars = []
#         for m in self._models:
#             if exclude_ex and m.interval.is_ex:
#                 pass
#             else:
#                 pars.append(m.parameters[par_name])
#
#         pars = np.array(pars)
#
#         if mode == "value":
...
#
#         raise ValueError("mode must be 'both', 'value', or 'error'")
#
#     # ------------------
#     # Semantic access
#     # ------------------
...
#             return cls(kind=EpisodeTypes.T90, start=float(m[1]), end=float(m[2]))
#
#         if m := cls._EX.match(s):
#             return cls(kind=EpisodeTypes(m[1]), start=float(m[2]), end=float(m[3]))
#
#         if m := cls._TR.match(s):
#             return cls(kind=EpisodeTypes.TR, index=int(m[1]), start=float(m[2]), end=float(m[3]))
#
...
#         return None
#
#     def get_model(self, model_name) -> ModelSet:
#         """Get models ordered as: t90 → ex → tr."""
#
#         t90_models = []
#         ex_models = []
#         tr_models = []
...
#
#                 if interval.is_t90:
#                     t90_models.append(model)
#                 elif interval.is_ex:
#                     ex_models.append(model)
#                 elif interval.is_tr:
#                     tr_models.append(model)
#
#         return ModelSet(t90_models + ex_models + tr_models)
#
#     def extract_interval_arrays(
#         self, return_diff: bool = False, return_mid_point: bool = False, return_all=False, exclude_ex=False
#     ):
#         """Extract start and end times as numpy arrays. Optionally return differences and midpoints."""
#         if exclude_ex:
#             st = np.array([i.start for i in self.intervals if not i.is_ex])
#             ed = np.array([i.end for i in self.intervals if not i.is_ex])
#         else:
#             st = np.array([i.start for i in self.intervals])
#             ed = np.array([i.end for i in self.intervals])
#
#         if return_diff:
#             diff = 0.5 * np.diff(a=[st, ed], axis=0)[0]
#             return st, ed, diff
#
#         if return_mid_point:
#             mid_point = (st + ed) / 2
#             return st, ed, mid_point
#
#         if return_all:
#             diff = 0.5 * np.diff(a=[st, ed], axis=0)[0]
#             mid_point = (st + ed) / 2
#             return st, ed, diff, mid_point
#
#         return st, ed
#
#     # ---------- container behavior ----------
...
#
#         return cls(
#             name, interval, {k: [v, e] for k, (v, e) in data.items()}, status, cstat, dof, CovarianceMatrix(cov_matrix)
#         )
#
#     def __str__(self) -> str:
#         return (
...
#         self, return_diff: bool = False, return_mid_point: bool = False, return_all=False, exclude_ex=False
#     ):
#         return self.intervals.extract_interval_arrays(return_diff, return_mid_point, return_all, exclude_ex)
#
#     def get_model(self, model_name) -> ModelSet:
#         return self.intervals.get_model(model_name)
#
#     def __repr__(self):
