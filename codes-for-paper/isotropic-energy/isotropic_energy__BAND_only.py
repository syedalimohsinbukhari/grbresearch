...
gc, grb_list_long, grb_objs, _ = prepare_grbs(grb_list, result_file, get_best=True)

grb_best = [i.get_model('BAND') for i in grb_objs]
...
amati_relationship_dirirsia2019(x_lim=(100, 3e4), y_lim=(1e51, 2e54), num_points=n_grid, use_average=True)
...
plt.tight_layout()
[plt.savefig(f"isotropic_energy__BAND_only.{i}", dpi=600) for i in ["png", "pdf"]]
plt.close()
# plt.show()
